...
describe('BookingGroup Add Person controls (e2e)', () => {
  ...
  let startClinicService: StartClinicService;
  ...
  beforeAll(async () => {
    ...
    startClinicService = moduleFixture.get(StartClinicService);
  ...
  it('adds exactly one new member, preserves Queue Numbers, and replays without duplicates', async () => {
    ...
    const [appointments, commands, outboxes, tokens] = await Promise.all([
    ...
    expect(commands[0]?.resultAppointmentId).toBe(firstBody.appointment.id);
    expect(outboxes).toHaveLength(1);
    expect(tokens).toHaveLength(1);
    expect(tokens[0]?.revokedAt).toBeNull();
  }, 30_000);

  it('rejects a sixth historical member without partial artifacts', async () => {
    const fixture = await createFixture(5);
    const beforeCounter = await queueCounter(fixture);
  ...
  it('serializes START CLINIC first, then rejects Add Person with no residue', async () => {
    ...
    try {
      ...
      const [startResult, addResult] = await Promise.allSettled([
        startPromise,
        addPromise,
      ]);
      ...
      if (addResult.status !== 'fulfilled') {
        throw addResult.reason;
      }
      expect(addResult.value.status).toBe(409);
    } finally {
  ...
  function queueCounter(fixture: Fixture) {
    return prisma.queueCounter.findUnique({
      where: {
        practiceLocationId_serviceDate: {
          practiceLocationId: fixture.locationId,
          serviceDate: fixture.serviceDate,
