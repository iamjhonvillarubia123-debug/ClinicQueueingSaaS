...
describe('Multi-person booking confirmation (e2e)', () => {
  ...
  it('atomically confirms two members, creates one group token/outbox, and replays without duplicate artifacts', async () => {
    ...
    const bookingGroupCookie = (firstCookies as unknown as string[]).find(
      (value) => value.startsWith('cq_booking_group_access='),
    );
    expect(bookingGroupCookie).toEqual(expect.any(String));
    expect(bookingGroupCookie).toContain('HttpOnly');
    expect(bookingGroupCookie).toContain('Secure');
    expect(bookingGroupCookie).toContain('SameSite=Strict');
    expect(bookingGroupCookie).toContain('Path=/patient-booking-groups');

    const persistedGroup = await prisma.bookingGroup.findFirstOrThrow({
      where: {
    ...
    const [
      ...
      prisma.bookingGroupAccessToken.findMany({
        where: { bookingGroupId: persistedGroup.id },
      }),
      prisma.notificationOutbox.findMany({
        where: {
          bookingGroupId: persistedGroup.id,
      ...
      prisma.contactPreference.findMany({
        where: {
          appointment: { bookingGroupId: persistedGroup.id },
        },
      }),
    ...
    expect(
      appointments.every((item) => item.bookingGroupId === persistedGroup.id),
    ).toBe(true);
    expect(
      appointments.every(
        (item) =>
    ...
    expect(commands[0]?.resultBookingGroupId).toBe(persistedGroup.id);
