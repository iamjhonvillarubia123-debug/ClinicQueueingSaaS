...
13. Once a BookingDraft is successfully converted into an Appointment, the BookingDraft is permanently consumed and browser-local draft continuity ends immediately. A consumed draft must never become editable or submittable again, even if browser Back/history displays stale cached booking UI.
14. Milestone 6 final conversion must be idempotent for repeated final submission of the same BookingDraft. A duplicate conversion command must return/reuse the already-created durable booking result rather than create another Appointment, allocate another Queue Number, or route the patient into recovery merely because the browser retried the same completed command.
15. If the patient deliberately starts a new booking from Home after a prior Appointment was already created, the approved duplicate-active-booking rule applies using the protected mobile + PracticeLocation + Service Date context. The public response before recovery OTP must remain neutral and must not reveal that an Appointment exists. The UI may offer Appointment Recovery, and only successful recovery verification may disclose/restore the existing Appointment. Appointment Recovery is not automatically invoked merely because a stale consumed draft page was reached through browser history.

## Closure reminder

Milestone 5 closure must reconcile older source wording that still mentions BookingQuestion duration adjustments with Amendment 1 above, must record the Version 1 browser-local-only BookingDraft continuity and no-draft-recovery policy in Amendments 8-12, must preserve the consumed-draft/idempotent-conversion/recovery boundary in Amendments 13-15, and must preserve the requirement that Milestone 6 final conversion revalidates current Service and BookingQuestion eligibility before durable Appointment creation.
