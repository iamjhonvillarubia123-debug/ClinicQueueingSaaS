...
export function VerifyEmailPage() {
  ...
  async function resend(event: FormEvent<HTMLFormElement>) {
    ...
    setResending(true);
    try {
      await apiRequest('/auth/resend-email-verification', { method: 'POST', body: { email: email.trim() } });
      setMessage('If this account is eligible for verification, a new verification email has been sent.');
      setState('idle');
    } catch (caught) {
      setMessage(messageFor(caught, 'Unable to request another verification email right now.'));
      setState('error');
    } finally {
      setResending(false);
    }
...
export function ForgotPasswordPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setError('');
    try {
      await apiRequest('/auth/request-password-reset', { method: 'POST', body: { email: email.trim() } });
      setSubmitted(true);
    } catch (caught) {
  ...
  return (
    <AccountFrame>
      <div className="auth-heading">
        ...
        <p>{submitted ? 'If an eligible account uses that email, a password-reset link has been sent.' : 'Enter your sign-in email. For security, this page will not confirm whether an account exists.'}</p>
      </div>
      {!submitted ? (
        <form className="stack" onSubmit={submit}>
          <label>Email<input type="email" required autoComplete="email" value={email} onChange={(event) => setEmail(event.target.value)} /></label>
          {error ? <div className="form-error" role="alert">{error}</div> : null}
          <button className="primary" type="submit" disabled={busy || !email.trim()}>{busy ? 'Sending…' : 'Send reset link'}</button>
        </form>
      ) : null}
  ...
  return (
    <AccountFrame>
      <div className="auth-heading">
        ...
        <p>{complete ? 'Your existing staff sessions have been revoked. Sign in again with the new password.' : 'A successful reset replaces the password and signs the account out of existing sessions.'}</p>
      </div>
      {complete ? <Link className="primary-action auth-full-action" to="/login">Sign in again</Link> : (
        <form className="stack" onSubmit={submit}>
          <label>New password<input type="password" required autoComplete="new-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label>
          <label>Confirm new password<input type="password" required autoComplete="new-password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} /></label>
          {error ? <div className="form-error" role="alert">{error}</div> : null}
