...
describe('F5 account access journeys', () => {
  it('registers a Doctor and moves to the email-verification gate without creating a session', async () => {
    ...
    expect(screen.getByText(/Public account creation is for Doctors/)).toBeInTheDocument();
    ...
    expect(fetchMock.mock.calls.some(([input]) => String(input).includes('/secretary/register'))).toBe(false);
