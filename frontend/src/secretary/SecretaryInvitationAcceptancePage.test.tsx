...
const preview = () => new Response(JSON.stringify({ name: 'Anna Cruz', email: 'anna@example.test', clinicName: 'North Clinic', expiresAt: '2026-09-07T00:00:00Z' }), { status: 200, headers: { 'content-type': 'application/json' } });
describe('SecretaryInvitationAcceptancePage', () => {
  it('lets the invited Secretary choose their own password', async () => {
    ...
    render(<MemoryRouter initialEntries={['/secretary-invitations/accept?token=valid-token']}><SecretaryInvitationAcceptancePage /></MemoryRouter>); expect(await screen.findByText(/invited to join/i)).toBeInTheDocument(); await user.type(screen.getByLabelText('Create Password'), 'Secretary password'); await user.type(screen.getByLabelText('Confirm Password'), 'Secretary password'); await user.click(screen.getByRole('button', { name: 'Create Secretary Account' })); await waitFor(() => expect(fetchMock).toHaveBeenCalledTimes(2)); expect(fetchMock.mock.calls[1][1]?.body).toBe(JSON.stringify({ token: 'valid-token', password: 'Secretary password' })); expect(await screen.findByRole('heading', { name: 'Account Created' })).toBeInTheDocument();
  });
