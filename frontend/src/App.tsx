...
function Shell() {
  ...
  const secretaryDraftOpen = profile?.role === 'SECRETARY' && location.pathname.startsWith('/app/secretary/settings-drafts/');
  return <div className="shell"><header className="appbar"><Link className="brand" to="/">Clinic Queueing</Link><div><span className="role">{profile?.role.replace('_', ' ')}</span>{profile?.role === 'DOCTOR' ? <Link className="quiet-link account-nav-link" to="/app/practice-locations">Clinics</Link> : null}{profile?.role === 'DOCTOR' ? <Link className="quiet-link account-nav-link" to="/app/secretary-draft-reviews">Reviews</Link> : null}{profile?.role === 'DOCTOR' ? <Link className="quiet-link account-nav-link" to="/app/defaults">Defaults</Link> : null}{profile?.role === 'DOCTOR' ? <Link className="quiet-link account-nav-link" to="/app/data-privacy">Data & Privacy</Link> : null}{profile?.role === 'SECRETARY' ? <Link className={`quiet-link account-nav-link${secretaryClinicsActive ? ' account-nav-active' : ''}`} to="/app/secretary/clinics">Clinics</Link> : null}{canManageOwnLifecycle ? <Link className="quiet-link account-nav-link" to="/app/account">Account</Link> : null}<button className="secondary" onClick={signOut}>Sign out</button></div></header><main className="workspace">{clinicTabs}{secretaryDraftOpen ? <div className="secretary-draft-exit"><Link className="secondary-action" to="/app/secretary/clinics">Cancel</Link></div> : null}<Outlet /></main></div>;
}
