...
export function IndividualBookingPage() {
  ...
  return <main className="public-detail"><PublicHeader /><article className="booking-flow">
    ...
    {stage === 'confirmed' && confirmation && <section className="booking-narrow confirmed-panel"><p className="eyebrow">Confirmed</p><h1>Your appointment is booked.</h1><div className="queue-confirmation"><span>Queue number</span><strong>{formatQueueNumber(confirmation.appointment.queueNumber)}</strong></div><div className="review-list"><div><span>Booking reference</span><strong>{confirmation.appointment.bookingReference}</strong></div><div><span>Service date</span><strong>{formatServiceDate(String(confirmation.appointment.serviceDate))}</strong></div></div><Link className="primary-action full-action" to={`/patient-bookings/${encodeURIComponent(confirmation.appointment.bookingReference)}`}>View appointment</Link><p className="field-note">Keep this booking available on this device. Access is protected by a secure browser cookie rather than a token shown in the URL.</p></section>}
  </article></main>;
}
