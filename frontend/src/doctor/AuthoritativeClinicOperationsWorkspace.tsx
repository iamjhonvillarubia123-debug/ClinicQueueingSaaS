    ...
    <div className="ops-overview">
      <div className="ops-overview-facts">
        <div>
          ...
          <span><small>Clinic Hours</small><strong>{hours}</strong></span>
        </div>
        <div>
        ...
        <div>
          ...
          <span><small>Patients in Queue</small><strong>{operations.queue.waitingCount}</strong></span>
        </div>
      ...
      <div className="ops-operating">
        <div>
          ...
          <span>
            ...
            <strong>{operations.clinicDay?.operatingSecretary?.name ?? 'Not assigned'}</strong>
            <small>For {formatServiceDate(serviceDate, true)}</small>
          </span>
        </div>
        <div>
          ...
          <span>
            ...
            <small>{operations.clinic.address || 'Address unavailable'}</small>
          </span>
        </div>
      </div>
      <div className="ops-overview-grid">
        <article className="ops-card ops-mini-queue">
          ...
          <div className="ops-current">
            <b>{operations.queue.nowServing ? formatQueueNumber(operations.queue.nowServing.queueNumber) : '—'}</b>
            <span><strong>{operations.queue.nowServing?.name ?? 'No patient is being served'}</strong><small>{operations.queue.nowServing?.serviceNames.join(', ') || '—'}</small></span>
          </div>
          <label>Next</label>
          <div className="ops-next">
            <b>{operations.queue.next ? formatQueueNumber(operations.queue.next.queueNumber) : '—'}</b>
            <strong>{operations.queue.next?.name ?? 'No waiting patient'}</strong>
            <span>{operations.queue.next?.serviceNames.join(', ') || '—'}</span>
          </div>
        </article>
        <div className="ops-overview-side">
          <article className="ops-card ops-appointment-glance">
            ...
            <div>
              ...
              <strong>{operations.appointments.total} <small>appointments</small></strong>
              <ul>
                <li>Waiting <b>{operations.appointments.counts.WAITING ?? 0}</b></li>
                <li>Now Serving <b>{operations.appointments.counts.CALLED ?? 0}</b></li>
                <li>Completed <b>{operations.appointments.counts.COMPLETED ?? 0}</b></li>
                <li>Cancelled <b>{operations.appointments.counts.CANCELLED ?? 0}</b></li>
              </ul>
            </div>
          </article>
...
function QueueTab({
  ...
  function handleDrawerCommand(command: QueueDrawerCommand) {
    if (command.type === 'STAFF_REINSERT') {
      return onEvent({ type: 'STAFF_REINSERT', patientId: command.appointmentId, afterPatientId: command.afterAppointmentId });
    }
  ...
  return (
    <div className="ops-queue">
      ...
      <div className="ops-queue-layout">
        <aside>
          <article className="ops-card ops-serving">
            <label>NOW SERVING</label>
            {current ? <div><b>{current.queue}</b><span><strong>{current.name}</strong><small>{current.service}</small><small>Est. service: {current.estimatedServiceMinutes} min</small></span></div> : <p>No patient is being served.</p>}
            {current ? <>
              <button type="button" className="ops-action is-green" onClick={() => setPatientOutcome('COMPLETED')}>COMPLETE</button>
              <button type="button" className="ops-action is-orange" onClick={() => setPatientOutcome('OUT_FOR_PROCEDURE')}>OUT FOR PROCEDURE</button>
              <button type="button" className="ops-action is-outline" onClick={() => setPatientOutcome('NOW_SERVING')}>DID NOT RESPOND</button>
            </> : null}
          </article>
        ...
        <main>
          <article className="ops-card ops-waiting-list">
            ...
            <div className="ops-table-head"><span>Queue #</span><span>Patient</span><span>Service</span><span>Waiting Since</span><span>Est. Service</span><span>Status</span></div>
            {waiting.map((row) => <div className="ops-table-row" key={row.id}><b>{row.queue}</b><strong>{row.name}</strong><span>{row.service}</span><span>{row.time}</span><span>{row.estimatedServiceMinutes} min</span><span>{row.status}</span></div>)}
            {!waiting.length ? <p>No patients are currently waiting.</p> : null}
          </article>
          <div className="ops-exception-grid">
            <article className="ops-card"><h3>TEMPORARILY ABSENT ({absent.length})</h3>{absent.map((row) => <div className="ops-exception" key={row.id}><b>{row.queue}</b><span><strong>{row.name}</strong><small>{row.service}</small></span><button type="button" onClick={() => onEvent({ type: 'RETURN_TO_QUEUE', patientId: row.id })}>RETURN TO QUEUE</button></div>)}</article>
            <article className="ops-card"><h3>OUT FOR PROCEDURE ({procedure.length})</h3>{procedure.map((row) => <div className="ops-exception" key={row.id}><b>{row.queue}</b><span><strong>{row.name}</strong><small>{row.service}</small></span><button type="button" onClick={() => onEvent({ type: 'RETURN_TO_QUEUE', patientId: row.id })}>RETURN TO QUEUE</button></div>)}</article>
          </div>
        </main>
      ...
      <div className="ops-action-bar">
        <button type="button" className="ops-action is-blue" onClick={() => setDrawer('walkin')}>ADD WALK-IN</button>
        <button type="button" className="ops-action is-green" disabled={!next} onClick={() => next && onEvent({ type: 'CALL_NEXT', patientId: next.id, patientOutcome })}>CALL NEXT</button>
        <button type="button" className="ops-action is-outline" onClick={() => setDrawer('adjust')}>ADJUST QUEUE</button>
        <button type="button" className="ops-action is-orange" onClick={() => setDrawer('delay')}>DELAY / BREAK</button>
      </div>
      {drawer ? (
        <QueueActionDrawer
          mode={drawer}
          ...
          onComplete={onNotice}
          patients={rows.map(({ id, queue, name, service, status }) => ({ id, queue, name, service, status }))}
    ...
    <div className="ops-appointments">
      ...
      <div className="ops-appointment-layout">
        <article className="ops-card ops-appointment-table">
          <header><h3>Appointments for {formatServiceDate(serviceDate, true)} <small>({filtered.length})</small></h3></header>
          <div className="ops-appt-head"><span>Queue #</span><span>Patient</span><span>Service(s)</span><span>Source</span><span>Status</span><span>View</span></div>
          {filtered.map((row) => <div className="ops-appt-row" key={row.id}><b>{row.queue}</b><span><strong>{row.name}</strong><small>{row.reference}</small></span><span><strong>{row.service}</strong></span><span>{row.source}</span><span>{row.status}</span><button type="button" aria-label={`View ${row.name}`} onClick={() => { setDetailsLoading(true); setDetailsError(''); void loadAppointmentDetails(row.id).then(setSelectedAppointment).catch((error: unknown) => setDetailsError(error instanceof Error ? error.message : 'Unable to load appointment details.')).finally(() => setDetailsLoading(false)); }}><OperationsIcon name="eye" size={18} /></button></div>)}
          {!filtered.length ? <p>No appointments match this service date and filter.</p> : null}
        </article>
        <aside>
          ...
          <article className="ops-card ops-pdf"><h3>Print / Save PDF</h3><p>PDF reporting remains a separate implementation checkpoint. No alternate report format is exposed here.</p></article>
        </aside>
      </div>
      ...
      {selectedAppointment ? (
        <AppointmentDetailsDrawer
          ...
          onReport={() => setDetailsError('PDF reporting is not available in this checkpoint.')}
        />
      ) : null}
    </div>
  );
