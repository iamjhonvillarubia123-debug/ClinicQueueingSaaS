...
export type StaffAssignmentCommand =
  ...
  | { role: 'INVITE_NEW'; firstName: string; lastName: string; email: string; mobileNumber: string };
...
export function StaffAssignmentDrawer({ data, pending, message, onClose, onSubmit }: { data: AuthoritativeClinicStaff; pending: boolean; message: string; onClose: () => void; onSubmit: (command: StaffAssignmentCommand) => void | Promise<void> }) {
  ...
  return <aside className="staff-assignment-drawer" aria-label="Assign Secretary drawer">
    ...
    <footer><button type="button" onClick={step === 1 ? onClose : () => setStep((value) => value - 1)}>{step === 1 ? 'Cancel' : 'Back'}</button>{mode === 'INVITE' && step === 2 ? <button type="button" className="is-primary" disabled={pending || !invite.firstName.trim() || !invite.lastName.trim() || !invite.email.trim() || !invite.mobileNumber.trim()} onClick={() => void onSubmit({ role: 'INVITE_NEW', ...invite })}>{pending ? 'Sending…' : 'Send Invitation'}</button> : step < 5 ? <button type="button" className="is-primary" disabled={(step === 2 && !userId) || (step === 4 && !configurationValid)} onClick={() => setStep((value) => value + 1)}>Next</button> : <button type="button" className="is-primary" disabled={pending} onClick={() => selected && void onSubmit(role === 'CLINIC_SECRETARY' ? { role, userId: selected.userId, authorityBundles: bundles, password: current ? password : undefined } : { role, userId: selected.userId, coverageMode, fromServiceDate: fromDate, toServiceDate: toDate })}>{pending ? 'Assigning…' : 'Confirm Assignment'}</button>}</footer>
  </aside>;
}
