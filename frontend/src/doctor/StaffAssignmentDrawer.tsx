  ...
  return (
    <aside className="staff-assignment-drawer" aria-label="Assign Secretary drawer">
      ...
      {step === 2 && mode === 'EXISTING' ? <>
        ...
        <label className="staff-candidate-search">Search your existing Secretaries<input type="search" placeholder="Search by name, email or mobile…" value={candidateSearch} onChange={(event) => setCandidateSearch(event.target.value)} /></label>
        <div className="staff-candidate-list">
          {visibleCandidates.length ? visibleCandidates.map((candidate) => <button type="button" key={candidate.userId} className={userId === candidate.userId ? 'is-selected' : ''} onClick={() => setUserId(candidate.userId)}><i aria-hidden="true" /><span><strong>{candidate.name}</strong><small>{candidate.email}</small><small>{candidate.mobileNumber}</small></span></button>) : <p>{candidateSearch.trim() ? 'No existing Secretary relationships match your search.' : 'No eligible existing Secretaries are available.'}</p>}
        </div>
      </> : null}
      {step === 2 && mode === 'INVITE' ? <>
        ...
        <div className="staff-invite-fields">
          ...
          <label>Email Address<input type="email" value={invite.email} onChange={(e) => setInvite({ ...invite, email: e.target.value })} /></label>
          <label>Mobile Number<input value={invite.mobileNumber} onChange={(e) => setInvite({ ...invite, mobileNumber: e.target.value })} /></label>
        </div>
      </> : null}
      {step === 3 ? <>
        <h2>Set Assignment Type</h2><p>Choose the type of assignment for {selectedName}.</p>
        <button type="button" className={`staff-choice-card ${role === 'CLINIC_SECRETARY' ? 'is-selected' : ''}`} onClick={() => setRole('CLINIC_SECRETARY')}><b>Clinic Secretary</b><span>Assign as the Clinic Secretary for {data.clinic.name}.</span></button>
        <button type="button" className={`staff-choice-card ${role === 'SUBSTITUTE_SECRETARY' ? 'is-selected' : ''}`} onClick={() => setRole('SUBSTITUTE_SECRETARY')}><b>Substitute Secretary</b><span>Temporary coverage with live clinic and queue authority only during covered dates.</span></button>
      </> : null}
      ...
      {step === 4 && role === 'SUBSTITUTE_SECRETARY' ? <>
        <h2>Substitute Secretary Coverage</h2><p>Select the inclusive coverage period for {selectedName}.</p>
        ...
        <div className="staff-date-fields"><label>From<input type="date" value={fromDate} onChange={(e) => { setFromDate(e.target.value); if (coverageMode === 'ONE_SERVICE_DATE') setToDate(e.target.value); }} /></label><label>To<input type="date" disabled={coverageMode === 'ONE_SERVICE_DATE'} value={toDate} onChange={(e) => setToDate(e.target.value)} /></label></div>
        <div className="staff-neutral-note">Authority is fixed and limited to live clinic and queue operations on covered Clinic Days. The Clinic Secretary remains assigned.</div>
      </> : null}
      {step === 5 ? <>
        <h2>{mode === 'INVITE' ? 'Review Invitation' : 'Review Assignment'}</h2><p>{mode === 'INVITE' ? 'Please review the pending clinic relationship invitation.' : 'Please review the clinic-scoped Secretary assignment.'}</p>
        <dl className="staff-review">
          <div><dt>Secretary</dt><dd>{selectedName}</dd></div>
          {mode === 'INVITE' ? <div><dt>Email</dt><dd>{invite.email}</dd></div> : null}
          <div><dt>Clinic</dt><dd>{data.clinic.name}</dd></div>
          <div><dt>Assignment Type</dt><dd>{role === 'CLINIC_SECRETARY' ? 'Clinic Secretary' : 'Substitute Secretary'}</dd></div>
          {role === 'CLINIC_SECRETARY' ? <div><dt>Authority Bundles</dt><dd>{bundles.map((bundle) => AUTHORITY_BUNDLES.find(([value]) => value === bundle)?.[1]).join(', ')}</dd></div> : <div><dt>Coverage Period</dt><dd>{fromDate} – {toDate}</dd></div>}
        </dl>
