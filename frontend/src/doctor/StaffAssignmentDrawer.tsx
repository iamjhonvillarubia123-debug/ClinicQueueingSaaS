...
export type StaffAssignmentCommand =
  ...
  | { role: 'INVITE_NEW'; firstName: string; lastName: string; email: string; mobileNumber: string; assignmentType: 'CLINIC_SECRETARY'; authorityBundles: string[]; requestedCancelClinicDay: boolean; password?: string }
...
export function StaffAssignmentDrawer({ data, pending, message, onClose, onSubmit }: {
  ...
  const selectedName = mode === 'INVITE' ? `${invite.firstName.trim()} ${invite.lastName.trim()}`.trim() : selected?.name;
  const detailsValid = Boolean(invite.firstName.trim() && invite.lastName.trim() && invite.email.trim() && invite.mobileNumber.trim());
  ...
  const accountEmailCorrectionVisible = Boolean(mode === 'INVITE' && step === 5 && messageIsError && (message.includes('Secretary account') || message.includes('email belongs to an account')));
  ...
  function submit() {
    if (mode === 'INVITE') {
      void onSubmit(role === 'CLINIC_SECRETARY'
        ? { role: 'INVITE_NEW', ...invite, assignmentType: role, authorityBundles: bundles, requestedCancelClinicDay: cancelClinicDay, password: current || cancelClinicDay ? password : undefined }
        : { role: 'INVITE_NEW', ...invite, assignmentType: role, coverageMode, fromServiceDate: fromDate, toServiceDate: toDate });
    } else if (selected) {
  ...
  return (
    <aside className="staff-assignment-drawer" aria-label="Assign Secretary drawer">
      ...
      {step === 2 && mode === 'INVITE' ? <><h2>Invitation Details</h2><p>Enter the Secretary details. The Secretary must already have an active, verified Secretary account before the clinic invitation can be sent.</p><div className="staff-invite-fields"><label>First Name<input value={invite.firstName} onChange={(e) => setInvite({ ...invite, firstName: e.target.value })} /></label><label>Last Name<input value={invite.lastName} onChange={(e) => setInvite({ ...invite, lastName: e.target.value })} /></label><label>Email Address<input type="email" value={invite.email} onChange={(e) => setInvite({ ...invite, email: e.target.value })} /></label><label>Mobile Number<input value={invite.mobileNumber} onChange={(e) => setInvite({ ...invite, mobileNumber: e.target.value })} /></label></div></> : null}
