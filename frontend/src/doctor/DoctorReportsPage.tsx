...
function Overview({ clinics }: { clinics: ReportClinic[] }) {
  return <>
...
function QueuePerformance({ clinics }: { clinics: ReportClinic[] }) {
  return <>
...
function ServicesReport({ clinics }: { clinics: ReportClinic[] }) {
...
export function ReportsDashboard({ clinics, loadError = false }: ReportsDashboardProps) {
  ...
  useEffect(() => {
    if (selectedClinicId !== 'ALL' && !clinics.some((clinic) => clinic.id === selectedClinicId)) setSelectedClinicId('ALL');
...
export function DoctorReportsPage() {
  ...
  useEffect(() => {
    let active = true;
    void apiRequest<ReportClinic[]>('/practice-location').then((items) => {
      if (!active) return;
