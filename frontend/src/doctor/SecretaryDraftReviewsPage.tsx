...
export function SecretaryDraftReviewPage() {
  ...
  return (
    <section className="practice-admin-page" aria-labelledby="review-heading">
      ...
      <section className="practice-create-panel" aria-labelledby="review-decision-heading"><div className="practice-panel-heading"><p className="eyebrow">Doctor decision</p><h2 id="review-decision-heading">Decide this submission</h2><p>Approve applies the proposal atomically after backend revalidation. Reject closes it. Return for rework reopens the same draft for the Secretary.</p></div><form className="practice-form" onSubmit={(event: FormEvent) => event.preventDefault()}><label>Note to Secretary <span className="optional">Optional</span><textarea maxLength={1000} value={comment} onChange={(event) => setComment(event.target.value)} /></label><div className="button-row"><button className="primary" type="button" disabled={Boolean(working)} onClick={() => void decide('approve')}>{working === 'approve' ? 'Approving…' : 'Approve and apply'}</button><button className="secondary" type="button" disabled={Boolean(working)} onClick={() => void decide('return-for-rework')}>{working === 'return-for-rework' ? 'Returning…' : 'Return for rework'}</button><button className="secondary" type="button" disabled={Boolean(working)} onClick={() => void decide('reject')}>{working === 'reject' ? 'Rejecting…' : 'Reject'}</button></div></form></section>
    </section>
  );
}
