...
export function AuthoritativeClinicOperationsRoutePage() {
  ...
  useEffect(() => {
    ...
    void apiRequest<QueueDrawerBookingConfiguration>(
      ...
      .then((result) => {
        if (!cancelled) setBookingConfiguration(result);
      })
      .catch(() => {
        if (!cancelled) setBookingConfiguration(null);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    void apiRequest<ClinicOperationsQueue>(
      `/practice-location/${encodeURIComponent(clinicId)}/operations/appointments?serviceDate=${encodeURIComponent(serviceDate)}`,
    )
      .then((result) => {
