...
describe('BookingDraftCleanupService', () => {
  ...
  it('runs expiration before protected-data cleanup', async () => {
    transactionMock.$queryRaw
      .mockResolvedValueOnce([])
      .mockResolvedValueOnce([]);

    await expect(
      service.runOnce(100, new Date('2026-08-17T15:00:00.000Z')),
    ).resolves.toEqual({ expired: 0, protectedDataCleared: 0 });
