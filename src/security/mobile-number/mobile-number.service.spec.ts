...
describe('MobileNumberService', () => {
  ...
  const lookupHmacKeyBase64 = Buffer.alloc(32, 2).toString('base64');
  ...
  beforeEach(async () => {
    ...
    service = module.get<MobileNumberService>(MobileNumberService);

    jest.clearAllMocks();
  });

  it('should return the canonical number and last four digits', () => {
    expect(service.normalize('+63 917 123 4567')).toEqual({
  ...
  it('should produce different ciphertext for repeated encryption of the same number', () => {
    const firstCiphertext = service.encryptCanonical('639171234567');

    const secondCiphertext = service.encryptCanonical('639171234567');
  ...
  it('should reject tampered ciphertext', () => {
    const encrypted = service.encryptCanonical('639171234567');

    const parts = encrypted.split('.');
    const ciphertext = parts[4];

    const tamperedCharacter = ciphertext.endsWith('A') ? 'B' : 'A';

    parts[4] = ciphertext.slice(0, -1) + tamperedCharacter;
  ...
  it('should protect a mobile number without returning the canonical plaintext', () => {
    ...
    expect(service.decrypt(result.encrypted)).toBe('639171234567');
  });
});
