...
describe('MobileNumberNormalizer', () => {
  ...
  it('should convert a local Philippine mobile number to canonical format', () => {
    expect(normalizer.normalize('09171234567')).toBe(
      '639171234567',
    );
  });
  ...
  expect(
    normalizer.normalize('639171234567'),
  ).toBe('639171234567');
  });
  ...
  expect(() =>
    normalizer.normalize('12345'),
  ).toThrow('Invalid Philippine mobile number.');
 });

 it('should normalize a formatted international mobile number', () => {
  expect(
    normalizer.normalize('+63 917 123 4567'),
  ).toBe('639171234567');
 });
