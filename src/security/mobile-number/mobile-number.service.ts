...
export class MobileNumberService {
  ...
  constructor(
    ...
    const encryptionKeyBase64 = this.configService.getOrThrow<string>(
      'MOBILE_ENCRYPTION_KEY_V1',
    );

    const lookupHmacKeyBase64 = this.configService.getOrThrow<string>(
      'MOBILE_LOOKUP_HMAC_KEY_V1',
    );

    const activeEncryptionKeyId = this.configService.getOrThrow<string>(
      'MOBILE_ENCRYPTION_ACTIVE_KEY_ID',
    );

    const activeLookupKeyId = this.configService.getOrThrow<string>(
      'MOBILE_LOOKUP_ACTIVE_KEY_ID',
    );

    const encryptionKey = Buffer.from(encryptionKeyBase64, 'base64');

    const lookupHmacKey = Buffer.from(lookupHmacKeyBase64, 'base64');
    ...
    if (!activeLookupKeyId.trim()) {
      throw new Error('MOBILE_LOOKUP_ACTIVE_KEY_ID must not be blank.');
    }

    this.encryptionKey = encryptionKey;
    this.lookupHmacKey = lookupHmacKey;
    this.activeEncryptionKeyId = activeEncryptionKeyId.trim();
  ...
  decrypt(envelope: string): string {
    ...
    try {
      ...
      throw new Error('Unable to decrypt protected mobile number.');
    }
  }
}
