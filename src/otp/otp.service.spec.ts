...
describe('OtpService', () => {
  ...
  const otpHmacKeyBase64 = Buffer.alloc(
    32,
    3,
  ).toString('base64');
  ...
  const configServiceMock = {
    getOrThrow: jest.fn((name: string) => {
      ...
      if (!value) {
        throw new Error(
          `Missing test configuration: ${name}`,
        );
      }
  ...
  $transaction: jest.fn(
    async (callback) =>
      callback({
        bookingDraft:
          prismaServiceMock.bookingDraft,
        otpVerification:
          prismaServiceMock.otpVerification,
      }),
  ),
};
...
it('should reject a malformed stored OTP hash', () => {
 ...
 it('should invalidate previous OTPs and create a new booking OTP', async () => {
  ...
  expect(
    prismaServiceMock.bookingDraft.findFirst,
  ...
  expect(
    prismaServiceMock.otpVerification.updateMany,
  ).toHaveBeenCalledWith({
    where: {
      bookingDraftId: 'draft-1',
      purpose: 'BOOKING',
  ...
  await expect(
    service.createBookingOtp('missing-draft'),
  ).rejects.toThrow(
