...
describe('PracticeLocationActivationService', () => {
  ...
  const activationDto = { practiceLocationId: 'location-1', currentPassword: 'current-password' };
  ...
  beforeEach(async () => {
    ...
    prismaServiceMock.practiceSchedule.findMany.mockResolvedValue([
      { weekday: Weekday.MONDAY, isOpen: true, opensAtLocal: time(9), closesAtLocal: time(17) },
    ]);
    scheduleResolutionMock.resolveConfiguredSchedule.mockResolvedValue({
      practiceLocationId: 'location-1', serviceDate: '2026-08-17', timeZone: 'Asia/Manila', isOpen: true,
      source: 'PRACTICE_SCHEDULE', opensAt: new Date('2026-08-17T01:00:00.000Z'), closesAt: new Date('2026-08-17T09:00:00.000Z'),
      maximumOnlineBookingUntilAt: null, maximumOperatingUntilAt: null,
    });
    doctorCalendarMock.isAvailableForInterval.mockResolvedValue(true);
    crossLocationConflictMock.assertNoConflictForInterval.mockResolvedValue(undefined);
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: DoctorCalendarAvailabilityService, useValue: doctorCalendarMock },
        { provide: CrossLocationScheduleConflictService, useValue: crossLocationConflictMock },
        { provide: RecurringScheduleConflictService, useValue: recurringScheduleConflictMock },
      ],
    }).compile();
  ...
  function arrangeLocation(status: PracticeLocationLifecycleStatus) {
    prismaServiceMock.$queryRaw
      .mockResolvedValueOnce([{ id: 'location-1', doctorProfileId: 'doctor-profile-1', doctorUserId: 'doctor-1', lifecycleStatus: status, name: 'Clinic A', addressLine1: '123 Main St', timeZone: 'Asia/Manila' }])
  ...
  it('activates a DRAFT location after recurring and concrete conflict validation', async () => {
    ...
    await expect(service.activate('doctor-1', activationDto, 'activate-key')).resolves.toEqual({ activated: true, replayed: false });
    expect(recurringScheduleConflictMock.assertNoConflictForLocation).toHaveBeenCalledWith('doctor-profile-1', 'location-1', 'Asia/Manila', prismaServiceMock);
    expect(prismaServiceMock.practiceLocation.update).toHaveBeenCalledWith({ where: { id: 'location-1' }, data: { lifecycleStatus: PracticeLocationLifecycleStatus.ACTIVE } });
    expect(crossLocationConflictMock.assertNoConflictForInterval).toHaveBeenCalledTimes(1);
  });
  ...
  it('blocks activation when recurring schedule validation conflicts', async () => {
    ...
    await expect(service.activate('doctor-1', activationDto, 'activate-recurring-conflict-key')).rejects.toBeInstanceOf(ConflictException);
  ...
  it('reactivates only location eligibility from DISABLED', async () => {
    ...
    await expect(service.reactivate('doctor-1', activationDto, 'reactivate-key')).resolves.toEqual({ reactivated: true, replayed: false });
    expect(prismaServiceMock.practiceLocation.update).toHaveBeenCalledWith({ where: { id: 'location-1' }, data: { lifecycleStatus: PracticeLocationLifecycleStatus.ACTIVE } });
  });
  ...
  it('requires at least one open recurring clinic schedule', async () => {
    arrangeLocation(PracticeLocationLifecycleStatus.DRAFT);
    prismaServiceMock.practiceSchedule.findMany.mockResolvedValue([]);
    await expect(service.activate('doctor-1', activationDto, 'activate-key')).rejects.toBeInstanceOf(ConflictException);
    expect(prismaServiceMock.practiceLocation.update).not.toHaveBeenCalled();
  });

  it('rejects activation when another active location has the same name and address', async () => {
    arrangeLocation(PracticeLocationLifecycleStatus.DRAFT);
    prismaServiceMock.practiceLocation.findFirst.mockResolvedValue({ id: 'location-2' });
    await expect(service.activate('doctor-1', activationDto, 'activate-key')).rejects.toBeInstanceOf(ConflictException);
  ...
  it('does not reactivate from a lifecycle state other than DISABLED', async () => {
    arrangeLocation(PracticeLocationLifecycleStatus.DRAFT);
    await expect(service.reactivate('doctor-1', activationDto, 'reactivate-key')).rejects.toBeInstanceOf(ConflictException);
    expect(prismaServiceMock.practiceLocation.update).not.toHaveBeenCalled();
  });
  ...
  it('replays a committed activation without repeating schedule effects', async () => {
    ...
    const requestFingerprint = createHash('sha256').update(`${CommandType.PRACTICE_LOCATION_ACTIVATE}|doctor-1|location-1`, 'utf8').digest('hex');
    prismaServiceMock.commandIdempotency.findUnique.mockResolvedValue({ requestFingerprint });
    await expect(service.activate('doctor-1', activationDto, 'activate-key')).resolves.toEqual({ activated: true, replayed: true });
    expect(prismaServiceMock.practiceSchedule.findMany).not.toHaveBeenCalled();
    expect(prismaServiceMock.practiceLocation.update).not.toHaveBeenCalled();
    expect(prismaServiceMock.commandIdempotency.create).not.toHaveBeenCalled();
  });
