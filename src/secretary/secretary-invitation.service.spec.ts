...
describe('SecretaryInvitationService', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    service = new SecretaryInvitationService(
      prisma,
      config,
      passwordSecurity,
      protectedPayload,
  ...
  it('creates only invitation and outbox state before acceptance', async () => {
    ...
    expect(transaction.secretaryInvitation.create).toHaveBeenCalledTimes(1);
  ...
  it('routes an existing Secretary account to assignment instead of creating an invitation', async () => {
  ...
  it('atomically creates the Secretary User and PracticeStaff only during acceptance', async () => {
    ...
    await expect(
      service.accept({ token, password: 'Secretary chosen password' }),
    ...
    expect(transaction.practiceStaff.create).toHaveBeenCalledTimes(1);
    expect(transaction.practiceLocation.update).toHaveBeenCalledWith(
      expect.objectContaining({ data: { currentRegularPracticeStaffId: 'staff-1' } }),
    );
