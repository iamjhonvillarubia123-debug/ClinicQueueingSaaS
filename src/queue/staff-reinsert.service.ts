...
@Injectable()
export class StaffReinsertService {
  constructor(
  ...
  async reinsert(
    ...
    return this.prisma.$transaction(async (transaction) => {
      ...
      const actor = await this.lockAndReadActor(
        transaction,
        authenticatedUserId,
      );
      ...
      if (!clinicDay || clinicDay.status !== ClinicDayStatus.STARTED) {
        throw new ConflictException(
          'STAFF REINSERT requires a started clinic day.',
        );
      }
      ...
      if (
        ...
        throw new ConflictException(
          'Appointment state does not permit Staff Reinsert.',
        );
      }
      ...
      const queueEvent = await transaction.queueEvent.create({
        data: {
          ...
          newPrimaryWaitingPlacementType: WaitingPlacementType.STAFF_REINSERT,
          createdAt: now,
        },
        select: { id: true },
      });
  ...
  private async readReplayResult(
    ...
    if (
      !event ||
      event.type !== QueueEventType.STAFF_REINSERTION ||
      ...
      throw new ConflictException(
        'STAFF REINSERT replay result is inconsistent.',
      );
    }
    ...
    if (!appointment) {
      throw new ConflictException(
        'STAFF REINSERT replay Appointment is unavailable.',
      );
    }
  ...
  private async lockAndReadActor(
    transaction: TransactionClient,
    userId: string,
  ): Promise<ActorState | null> {
    const rows = await transaction.$queryRaw<ActorState[]>(Prisma.sql`
