-- R3 account-mobile OTP notification scope reconciliation.
-- Account-level mobile verification is not PracticeLocation-scoped.
-- Preserve existing clinic-scoped OTP behavior while allowing an
-- OtpVerification-backed account OTP to have no PracticeLocation.
...
ALTER TABLE "NotificationOutbox"
  ...
  CHECK (
    ...
    OR (
      ...
      AND "practiceLocationId" IS NULL
      AND "otpVerificationId" IS NOT NULL
    )
    OR (
      "notificationType" IN (
